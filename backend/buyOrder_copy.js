...
handle_request = async (data, callback) => {
  ...
  try {
    ...
    let matchq = "select SellOrder.order_id, SellOrder.num, SellOrder.suser_id from BuyOrder, SellOrder, Stock where BuyOrder.order_id=" +
      ...
      "BuyOrder.buser_id != SellOrder.suser_id and " +
    ...
    for (i = 0; i < matcharr.length; i++) {
      console.log("i:" + i + ", matcharr:" + matcharr[i][0]);
      ...
      // 更新掛單數量
      let updateSellOrderNum = "update SellOrder set num=(" + matcharr[i][1] + "-" + min + ") where order_id=" + matcharr[i][0];
      ...
      console.log("curBuyNum_2: " + curBuyNum);

      // 更新交易紀錄
      trans_id = orderID + data.stock_id + matcharr[i][0];
      console.log("index: " + i);
      ...
      let transact = "insert into MyTransaction (transaction_id, tbuy_order_id, tsell_order_id, finish_time, tstock_id, num, price) values (" +
        trans_id + ", " + orderID + ", " + matcharr[i][0] + ", curTime(), " + data.stock_id + ", " + min + ", " + newprice + ");";
      ...
      console.log("index: " + i);
      let sell_own_now = 0;
      let get_sell_own = "select num from Own where Ouser_id=" + matcharr[i][2] + " and Ostock_id=" + data.stock_id + ";";
      ...
      let sell_own = "update Own set num=(" + sell_own_now + "-" + min + ") where Ouser_id=" + matcharr[i][2] + " and Ostock_id=" + data.stock_id + ";";
      ...
      console.log("index: " + i);
      let sell_money = 0;
      let get_sell_money = "select money from User where user_id=" + matcharr[i][2] + ";";
      ...
      let cfc_sell = "update User set money=(" + sell_money + "+" + cashflow + ") where user_id=" + matcharr[i][2] + ";";
