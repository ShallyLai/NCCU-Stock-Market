...
handle_request = async (data, callback) => {
  ...
  try {
    ...
    for(i = 0; i < matcharr.length; i++){
      ...
      // 更新掛單數量
      let updateBuyOrderNum = "update BuyOrder set num=(" + matcharr[i][1] + "-" + min + ") where order_id=" + matcharr[i][0];
      ...
      let transact = "insert into MyTransaction (transaction_id, tbuy_order_id, tsell_order_id, finish_time, tstock_id, num, price) values (" +
        trans_id + ", " + matcharr[i][0] + ", " + orderID + ", curTime(), " + data.stock_id + ", " + min + ", " + newprice + ");";
      ...
      console.log("index: " + i);
      let buy_own_now = 0;
      let get_buy_own = "select num from Own where Ouser_id=" + matcharr[i][2] + " and Ostock_id=" + data.stock_id + ";";
      ...
      let buy_own = "update Own set num=(" + buy_own_now + "+" + min + ") where Ouser_id=" + matcharr[i][2] + " and Ostock_id=" + data.stock_id + ";";
      ...
      let cfc_buy = "update User set money=(" + buy_money + "-" + cashflow + ") where user_id=" + matcharr[i][2] + ";";
